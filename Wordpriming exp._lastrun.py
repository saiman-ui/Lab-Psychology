...
expName = 'Wordpriming exp.'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\DELL\\Desktop\\Word Priming\\Wordpriming exp._lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('C:/Users/DELL/Downloads/StudyPhase_words.xlsx'), 
        seed=None, 
    )
    ...
    trials_2 = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('TestPhase_words_with_fragments.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial_2 in trials_2:
        ...
        # create an object to store info about Routine Test_Routine
        Test_Routine = data.Routine(
            name='Test_Routine',
            components=[text_2, textbox, text_3, mouse],
        )
        Test_Routine.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        text_2.setText(fragment
        ...
        # store start times for Test_Routine
        Test_Routine.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        Test_Routine.tStart = globalClock.getTime(format='float')
        Test_Routine.status = STARTED
        thisExp.addData('Test_Routine.started', Test_Routine.tStart)
        Test_Routine.maxDuration = None
        # keep track of which components have finished
        Test_RoutineComponents = Test_Routine.components
        for thisComponent in Test_Routine.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        Test_Routine.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                Test_Routine.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in Test_Routine.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in Test_Routine.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for Test_Routine
        Test_Routine.tStop = globalClock.getTime(format='float')
        Test_Routine.tStopRefresh = tThisFlipGlobal
        thisExp.addData('Test_Routine.stopped', Test_Routine.tStop)
        ...
        if Test_Routine.maxDurationReached:
            routineTimer.addTime(-Test_Routine.maxDuration)
        elif Test_Routine.forceEnded:
            routineTimer.reset()
        else:
